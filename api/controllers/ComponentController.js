...
module.exports = {
	getById: function(req, res){
		...
		Component.find({id: id}).populate('gpios')
	...
	create: function(req, res, next){
		...
		if(!type){
			res.json({ status: 'failed', error: 'The "{type:<switch|switchBlind|switchAudio|dimmer|socket|testAC|temperatureSensor|motionSensor|luminanceSensor>}" is required.'});
			return next();
		}
		...
		switch (obj.type) {
			case 'socket':
				if(!number1){
					res.json({ status: 'failed', error: 'The "{number1:<number1>}" is required.'});
					return next();
				}
				this._createSocket(gpios)
				.catch(function(e){
					res.json({ status: 'failed', error: e.message});
				})
				.then(function(result){
			...
			case 'temperatureSensor':
				...
				this._createTemperatureSensor(gpios)
				.catch(function(e){
					res.json({ status: 'failed', error: e.message});
				})
				.then(function(result){
			...
			case 'motionSensor':
					...
					this._createMovementSensor(gpios)
					.catch(function(e){
						res.json({ status: 'failed', error: e.message});
					})
					.then(function(result){
			...
			case 'switch':
				...
				if(!number3){
					res.json({ status: 'failed', error: 'The "{number3:<number3>}" is required.'});
					return next();
				}
				this._createSwitch(gpios)
				.catch(function(e){
					res.json({ status: 'failed', error: e.message});
				})
				.then(function(result){
			...
			case 'switchBlind':
				if(!number1){
					res.json({ status: 'failed', error: 'The "{number1:<number1>}" for up mode is required.'});
					return next();
				}
				if(!number2){
					res.json({ status: 'failed', error: 'The "{number2:<number2>}" for down mode is required.'});
					return next();
				}
				this._createSwitchBlind(gpios)
				.catch(function(e){
					res.json({ status: 'failed', error: e.message});
				})
				.then(function(result){
			...
			case 'luminanceSensor':
				...
				if(number1Type !== 'analog'){
					res.json({ status: 'failed', error: 'The "number1Type" should be "analog" .'});
					return next();
				}



				this._createLuminanceSensor(gpios)
				.catch(function(e){
					res.json({ status: 'failed', error: e.message});
				})
				.then(function(result){
	...
	destroy: function(req, res){
		...
		.then(function(component){
			...
			if(component.gpios.length === 1){
					return Gpio.update(component.gpios[0].id, {owner: null});
			}
			if(component.gpios.length === 2){
					return Gpio.update(component.gpios[0].id, {owner: null})
					.then(function(){
						return Gpio.update(component.gpios[1].id, {owner: null});
					});
			}
			if(component.gpios.length === 3){
					...
					.then(function(){
						return Gpio.update(component.gpios[1].id, {owner: null})
						.then(function(){
							return Gpio.update(component.gpios[2].id, {owner: null});
						});
					});
			}
		...
		.then(function(obj){
			...
			return Component.destroy({id: id});
		})
		.catch(function(e){
			res.json({ status: 'failed', error: e.message});
		})
	...
	_createSwitch: function(obj){
		return this._createComponentGpio('switch', obj.name, obj.ip, {number: obj.number1, direction: 'out', type: 'digital', action: 'switch'}, {number: obj.number2, direction: 'out', type: 'digital', action: 'switch'}, {number: obj.number3, direction:'in', type: 'digital', action: 'testAC'});
	},
	//{ip, number1, number2}
	_createSwitchBlind: function(obj){
		return this._createComponentGpio('switchBlind', obj.name, obj.ip, {number: obj.number1, direction: 'out', type: 'digital', action:'up'}, {number: obj.number2, direction: 'out', type: 'digital', action:'down'});
	},
	//{ip, number1}
	_createSwitchAudio: function(obj){
		return this._createComponentGpio('switchAudio', obj.name, obj.ip, {number: obj.number1, direction: 'out', type: 'digital'});
	},
	//{ip, number1}
	_createDimmer: function(obj){
		//return this._createComponentGpio('dimmer', obj.name, obj.ip, {number: obj.number1, direction:'out', type: 'digital', action: 'dimmer'}, {number: obj.number2, direction:'in', type: 'digital', action: 'testAC'});
		return this._createComponentGpio('dimmer', obj.name, obj.ip, {number: obj.number1, direction:'out', type: 'digital', action: 'dimmer'});
	},
	//{ip, number1}
	_createSocket: function(obj){
		return this._createComponentGpio('socket', obj.name, obj.ip, {number: obj.number1, direction: 'out', type: 'digital'});
	},
	//{ip, number1}
	_createTestAC: function(obj){
		return this._createComponentGpio('testAC', obj.name, obj.ip, { number: obj.number1, direction: 'in', type: 'digital', action: 'testAC'});
	},
	//{ip, number1}
	_createTemperatureSensor: function(obj){
		return this._createComponentGpio('temperatureSensor', obj.name, obj.ip, { number: obj.number1, direction:'out', type: 'digital'});
	},
	_createMotionSensor: function(obj){
		return this._createComponentGpio('motionSensor', obj.name, obj.ip, { number: obj.number1, direction:'out', type: 'digital' });
	},
	_createLuminanceSensor: function(obj){
		return this._createComponentGpio('luminanceSensor', obj.name, obj.ip, { number: obj.number1, direction:'out', type: 'analog'});
	},
	_createComponentGpio: function(componentType, name, ip, gpio1, gpio2, gpio3 ){
		...
		return this._checkGpios(ip, gpio1, gpio2, gpio3)
		.then(function(){
			...
			return Component.create({ type: componentType, name: name });
		})
		.then(function(item){
			componentId = item.id;

			if(number1 && !number2 && !number3){
				return Gpio.findOne({ number: number1, ip: ip, type: type1 })
				.then(function(found){
					found.direction = direction1;
					found.action = action1;
					found.type = type1;
					found.owner = componentId;
					return Gpio.update(found.id, found);
				});
			}
			if(number1 && number2 && !number3){
				return Gpio.findOne({ number: number1, ip: ip, type: type1 })
				...
				.then(function(){
					return Gpio.findOne({ number: number2, ip: ip, type: type2 });
				})
				.then(function(found){
					if(!found){
						throw new Error('The gpio'+found.number+' (' + found.ip + ')  not found.');
					}
					found.direction = direction2;
					found.action = action2;
					found.type = type2;
					found.owner = componentId;
					return Gpio.update(found.id, found);
				});
			...
			if(number1 && number2 && number3){
				...
				.then(function(){
					return Gpio.findOne({ number: number3, ip: ip, type: type3 });
				})
				.then(function(found){
					if(!found){
						throw new Error('The gpio'+found.number+' (' + found.ip + ')  not found.');
					}
					found.direction = direction3;
					found.action = action3;
					found.type = type3;
					found.owner = componentId;
					return Gpio.update(found.id, found);
				});
	...
	_checkGpios: function(ip, gpio1, gpio2, gpio3){
		...
		if(numberToCheck1 && !numberToCheck2 && !numberToCheck3){
			...
			return Gpio.findOne({ ip: ip, number: numberToCheck1, type: gpio1Type})
			.then(function(gpio){
		...
		if(numberToCheck1 && numberToCheck2 && !numberToCheck3){
			...
			.then(function(gpio1){
				...
				return Gpio.findOne({ ip: ip, number: numberToCheck2, type: gpio2Type});
			})
			.then(function(gpio2){
				if(!gpio2){
		...
		if(numberToCheck1 && numberToCheck2 && numberToCheck3){
			...
			.then(function(gpio2){
				...
				return Gpio.findOne({ ip: ip, number: numberToCheck3, type: gpio3Type});
			})
			.then(function(gpio3){
				if(!gpio3){
